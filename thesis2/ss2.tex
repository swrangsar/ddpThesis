...
spectrum holes which can be utilized by secondary users for communication and to enable secondary users to quit the 
frequency band as soon as the corresponding primary radio emerges. This technique is called spectrum 
...
bandwidth $W$ and comparing this statistics to a predetermined threshold $\gamma$. In our project energy 
...
Figure \ref{compSensingTch} compares various spectrum sensing techniques on the basis of their 
...
It is used as a basic code for wide band spectrum analyzer implementation. The output of this 
code is the magnitude squared of the FFT. This means for each FFT bin the output 
...
$(bin0 == X[0])$ is the passband centre frequency The first half of FFT spectrum ($X[1]$ to $X[N/2-1]$) contains the positive baseband frequencies, which corresponds
